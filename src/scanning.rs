use std::{ops::RangeInclusive, net::Ipv4Addr};
...
impl IPV4AddressTemplate {
	...
	pub fn from_netmask(network_prefix: Ipv4Addr, subnet_mask_bit_length: usize, port: TemplateMember<u16>) -> Self {
		debug_assert!(subnet_mask_bit_length <= 32, "subnet_mask_bit_length has to be <= 32");
...
mod tests {
	#[test]
	fn netmask() {
		use super::IPV4AddressTemplate;
		use std::net::Ipv4Addr;
		let t = IPV4AddressTemplate::from_netmask(
			Ipv4Addr::new(10, 201, 1, 0),
			24,
			super::TemplateMember::Fixed(80)
		);
		for ip in t.into_iter() {
			eprintln!("{ip}");
		}
	}
}
